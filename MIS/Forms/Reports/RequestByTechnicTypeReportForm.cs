...
{
    public partial class RequestByTechnicTypeReportForm : Form
    {
        private readonly Repository _repository = Repository.RepositoryInstance;

        public RequestByTechnicTypeReportForm()
        {
            InitializeComponent();
        }
        ...
        private void UpdateDatagrid()
        {
            ...
            requestByTechnicTypeBindingSource.DataSource = null;
            requestByTechnicTypeBindingSource.DataSource =
                _repository.GetRequestGroupBy<Request, TechnicType>(request => request.Technic.TechnicType, dateFrom, dateTo) // выбираем по датам и группируем по типу несиправности
            // выбираем из группированной коллекции тип неисправности и количество заявок
                    .Select(requests => new RequestByTechnicType() { TechnicType = requests.Key, Count = requests.Count() })
            .ToList();
        }
        ...
        private void buttonExportToWord_Click(object sender, EventArgs e)
        {
            ...
            var data = requestByTechnicTypeBindingSource.DataSource as List<RequestByTechnicType>;
            if (data != null && data.Count > 0)
            {
                using (var export = new ExportToWord())
                {
                    export.CreateRequestsByTechnikTypesReport(data, dateFrom, dateTo);
                }
            }
            else
            {
